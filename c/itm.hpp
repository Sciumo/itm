...
double DllExport ascat(double d, prop_type &prop, propa_type &propa);

double DllExport adiff(double d, prop_type &prop, propa_type &propa);

void DllExport qlrps(double fmhz, double zsys, double en0,
                             int ipol, double eps, double sgm, prop_type &prop);

double DllExport alos(double d, prop_type &prop, propa_type &propa);

void DllExport qlra(int kst[], int klimx, int mdvarx,
                            prop_type &prop, propv_type &propv);


void DllExport lrprop(double d,
                              prop_type &prop, propa_type &propa);

double DllExport avar(double zzt, double zzl, double zzc,
                              prop_type &prop, propv_type &propv);

void DllExport hzns(double pfl[], prop_type &prop);
void DllExport z1sq1(double z[], const double &x1, const double &x2,
                             double &z0, double &zn);
double DllExport qtile(const int &nn, double a[], const int &ir);

double DllExport d1thx(double pfl[], const double &x1, const double &x2);

void DllExport qlrpfl(double pfl[], int klimx, int mdvarx,
                              prop_type &prop, propa_type &propa, propv_type &propv);

void DllExport point_to_point(double elev[], double tht_m, double rht_m,
                                      double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
                                      double frq_mhz, int radio_climate, int pol, double conf, double rel,
                                      double &dbloss, char *strmode, int &errnum);

void DllExport point_to_pointMDH(double elev[], double tht_m, double rht_m,
                                         double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
...
void DllExport area(long ModVar, double deltaH, double tht_m, double rht_m,
...
void DllExport point_to_pointDH(double elev[], double tht_m, double rht_m,
                                        double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
                                        double frq_mhz, int radio_climate, int pol, double conf, double rel,
                                        double &dbloss, double &deltaH, int &errnum);
double DllExport ITMAreadBLoss(long ModVar, double deltaH, double tht_m, double rht_m,
                                       double dist_km, int TSiteCriteria, int RSiteCriteria,
                                       double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
                                       double frq_mhz, int radio_climate, int pol, double pctTime, double pctLoc,
                                       double pctConf);

double  DllExport ITMDLLVersion();
